...
public class TicketResource {
    ...
    public ResponseEntity<List<Ticket>> searchTickets(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Tickets for query {}", query);
        Page<Ticket> page = ticketSearchRepository.search(queryStringQuery(query), pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
